...
void construct_op_response(const char* request, const char* request_end, khash_t(header) * hash_table, char* request_result)
{
    char* header_start = NULL;
    char* header_end = NULL;
        ...
        if(res_size > 1 && result[0] != NULL && result[1] != NULL){
            ...
            if(k != kh_end(hash_table)){
               ...
               if(strcmp(kh_value(hash_table,k) , result[1]) == 0){
                   ...
                   /* fprintf(stderr, "=================? %s\n",result[0]); */
                   /* if(strcmp(result[0], "Set-Cookie") != 0){ */
                       request_result[r_index] = '$';
                       r_index++;
                       memcpy(request_result + r_index,header_start, header_end - header_start);
                       r_index += header_end - header_start;
                   /* } */
               }
            }else{
    ...
    if(strstr(data,"POST /document/save?id=")){
        ...
        if(it == kh_end(global_hlist->list)){
            ...
            *hte = new_entry;
            ...
            //alread store;
            fprintf(stderr, "request is already stored, maybe some error hanppend\n");
            goto leave;
        }
 ...
 *         } */
        ...
        construct_op_response(p,end + 2,entry->rp_ht,header_result + p_ret + 1);
        size_t r_size = strlen(header_result) + payload_size + 4;//4 for "/r/n/r/n"
        result = malloc(r_size);
        memset(result,'\0',r_size);
        memcpy(result,header_result,strlen(header_result));
...
int get_iterator_from_request(char* data, size_t start,size_t end,int* forward)
{
    size_t index = 0;
    int result = 0;
    while(data[start + index] != '|' && start + index < end){
...
char* reconstruct_googledoc_request(char* data, size_t length,ht_entry** entry,int *o_size)
{
    ...
    if(data_tmp[0] == '*')
    {
        ...
        /* fprintf(stderr, "header_result %lu payload_size %lu\n",header_size,payload_size); */
        *o_size = header_size + payload_size + 1;
        /* fprintf(stderr, "header_size: %zu payload_size: %zu\n",header_size,payload_size); */
        return result;
    }
    return NULL;
}
